import type { ArrowPair, SavedDFA, State } from "../types/types";
import type {
  AutomatonInit,
  RegexFragment,
  RegexToken,
...
const sortedNumbers = (values: Iterable<number>) => [...values].sort((a, b) => a - b);
...
export class FiniteAutomaton {
  ...
  static fromCanvasSnapshot(states: State[], arrowPairs: ArrowPair[], alphabet: string[] = []) {
  ...
  getType() {
    ...
    if (!this.isCompleteDeterministic()) return "Incomplete DFA" as const;
    return "DFA" as const;
  }
    ...
    this.transitions.forEach((symbolMap, from) => {
      symbolMap.forEach((targets, symbol) => {
        targets.forEach(to => {
          ...
          arrowPairs.push({ from: fromIndex, to: toIndex, label: symbol });
        });
      });
    });
...
export const automatonFromSavedFA = (savedFA: SavedDFA, fallbackAlphabet: string[] = []) => {
